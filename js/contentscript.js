...
const isMainPage = setInterval(function() {
      if (!document.getElementsByClassName('profiles-gate-container')[0]) {
          ...
          function checkLanguage() {
            const checkLang = document.getElementsByClassName('browse')[0].getElementsByTagName('a')[0].textContent;
...
function createHiddenGem(data) {
  ...
  const extBrowse = document.createElement('a');
  extBrowse.setAttribute('class', 'extBrowse');
  extBrowse.textContent = data.menu[0].browse;  //Get translated button text from JSON file.
  ...
  extBrowsePlacement.insertBefore(extBrowse, kids);
  ...
  extBrowsePlacement.insertBefore(container, kids);
